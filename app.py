...
def export():
    ...
    tmp_files = []  # temp image files — cleaned up AFTER wb.save()
    for lvsb in export_list:
        ...
        for i, raw_img in enumerate(entry.get('images', [])[:12]):
            try:
                # Handle both dict {'fname':..,'cloudinary_url':..} and plain string
                ...
                if img_data:
                    # Keep temp files alive until wb.save() — collect for cleanup after
                    ...
                    try:
                        insert_image_exact(ws, tmp.name, SLOT_LAYOUT[i])
                    except Exception as e:
                        print(f'[Export] insert error {fname}: {e}')
                else:
                    ...
                    if os.path.exists(fpath):
                        try:
                            ...
                            print(f'[Export] local insert error {fname}: {e}')
            except Exception as e:
    ...
    try:
        ...
        # Now safe to delete temp files — openpyxl has finished reading them
        for tf in tmp_files:
            try:
