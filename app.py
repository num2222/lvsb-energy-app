...
def _cloudinary_meta_id(mk):
    return f'lvsb_meta/{mk}'

def _fetch_meta_from_cloudinary(mk):
    """Download meta JSON stored as raw file on Cloudinary"""
    try:
        import cloudinary
        ...
        # Raw files stored under lvsb_meta/ folder as .json
        ...
        pass  # not found or error — return None
    return None

def _push_meta_to_cloudinary(mk, meta_dict):
    """Upload meta JSON as raw file to Cloudinary"""
    try:
        import cloudinary
        import cloudinary.uploader
        cloudinary.config(
        ...
        data = json.dumps(meta_dict, ensure_ascii=False).encode('utf-8')
        cloudinary.uploader.upload(
            data,
            public_id=f'lvsb_meta/{mk}',
            resource_type='raw',
            overwrite=True,
        )
    except Exception as e:
        print(f'[Meta] Cloudinary push error: {e}')
...
def load_meta(mk=None):
    ...
    if os.path.exists(mf):
        with open(mf, 'r', encoding='utf-8') as f:
            return normalize_meta(json.load(f))

    # 2. Try Cloudinary (persistent across restarts)
    if cloudinary_enabled():
        cloud_meta = _fetch_meta_from_cloudinary(mk)
        if cloud_meta:
            cloud_meta = normalize_meta(cloud_meta)
            # Cache locally for this session
            os.makedirs(os.path.dirname(mf), exist_ok=True)
            with open(mf, 'w', encoding='utf-8') as f:
                json.dump(cloud_meta, f, ensure_ascii=False)
            print(f'[Meta] Restored from Cloudinary: {mk}')
            return cloud_meta
...
def normalize_meta(meta):
    """Ensure all image entries are dicts, not plain strings"""
    for lid, entry in meta.get('lvsbs', {}).items():
        ...
        for img in imgs:
            if isinstance(img, dict):
                ...
                normalized.append({'fname': img, 'cloudinary_url': None, 'drive_id': None})
        entry['images'] = normalized
    return meta
...
def save_meta(meta, mk=None):
    ...
    # Push to Cloudinary for persistence
    if cloudinary_enabled():
        _push_meta_to_cloudinary(mk, meta)
    ...
    if cloudinary_enabled():
        try:
            ...
            result = cloudinary.api.resources(
                ...
                prefix='lvsb_meta/', max_results=50
            )
            for r in result.get('resources', []):
                pid = r['public_id'].replace('lvsb_meta/', '')
                if '_' in pid:
                    months.add(pid)
        except Exception as e:
            pass
...
def insert_image_exact(ws, img_source, slot):
    """img_source can be a file path (str) or bytes"""
    if isinstance(img_source, bytes):
        xl_img = XLImage(io.BytesIO(img_source))
    else:
...
def serve_image(mk, lid, fname):
    meta     = load_meta(mk)
    entry    = meta.get('lvsbs', {}).get(lid, {})
    img_info = next((i for i in entry.get('images', [])
                     if (i['fname'] if isinstance(i, dict) else i) == fname), None)
    # Try Cloudinary URL first (redirect)
...
@app.route('/export')
def _fill_images(ws, entry, lid, mk):
    """Download and insert images into worksheet"""
    logo = ws._images[0] if ws._images else None
    ws._images = []
    if logo:
        ws._images.append(logo)
    for i, raw_img in enumerate(entry.get('images', [])[:12]):
        try:
            if isinstance(raw_img, dict):
                fname          = raw_img.get('fname', '')
                cloudinary_url = raw_img.get('cloudinary_url')
                drive_id       = raw_img.get('drive_id')
            else:
                fname, cloudinary_url, drive_id = str(raw_img), None, None
            if not fname:
                continue
            img_data = None
            if cloudinary_url:
                img_data = download_from_cloudinary(cloudinary_url)
                print(f'[Export] cloudinary {fname}: {"OK" if img_data else "FAIL"}')
            elif drive_id:
                img_data = download_from_drive(drive_id)
            if img_data:
                insert_image_exact(ws, img_data, SLOT_LAYOUT[i])
            else:
                fpath = os.path.join(img_dir(lid, mk or 'default'), fname)
                if os.path.exists(fpath):
                    with open(fpath, 'rb') as f:
                        insert_image_exact(ws, f.read(), SLOT_LAYOUT[i])
        except Exception as e:
            print(f'[Export] skip img {i} lid={lid}: {e}')
...
def _do_export():
    mth      = request.args.get('month_th', '')
    yr       = request.args.get('year_th', '')
    lid_only = request.args.get('lid', '')   # ถ้ามีค่า = Export แยกตู้
    mk       = month_key(mth, yr) if (mth and yr) else None
    meta     = load_meta(mk)
    ...
    if lid_only:
        # ── Export แยกตู้: มีแค่ Main + Sheet ตู้นั้น ──────────────────
        lvsb = next((l for l in LVSBS if l['id'] == lid_only), None)
        if not lvsb:
            return 'LVSB not found', 404

        wb      = openpyxl.load_workbook(TEMPLATE_PATH)
        ws_main = wb['Main']
        ws_main['A1'] = f'แบบแผนงานสำรวจ  LVSB  ประจำเดือน  {month_th}   {year_th}'

        # อัปเดต Main เฉพาะแถวของตู้นี้
        entry   = meta.get('lvsbs', {}).get(lid_only, {})
        row_idx = int(lid_only) + 3
        ...
        # ลบ Sheet ที่ไม่ต้องการออก — เหลือแค่ Main กับ Sheet ตู้นี้
        keep = {'Main', lvsb['sheet']}
        for sname in list(wb.sheetnames):
            if sname not in keep:
                del wb[sname]

        # ใส่รูปใน Sheet ตู้นี้
        if lvsb['sheet'] in wb.sheetnames:
            ws = wb[lvsb['sheet']]
            if date_val:
                try:
                    d = datetime.strptime(date_val, '%Y-%m-%d')
                    ...
                    ws['C7'] = date_val
            _fill_images(ws, entry, lid_only, mk)

        lvsb_name = lvsb['name']
        fname_dl  = f'Energy_{lvsb_name}_{month_th}_{year_th}.xlsx'

    else:
        # ── Export ทั้งหมด: อัปเดตทุก Sheet ──────────────────────────
        wb      = openpyxl.load_workbook(TEMPLATE_PATH)
        ws_main = wb['Main']
        ws_main['A1'] = f'แบบแผนงานสำรวจ  LVSB  ประจำเดือน  {month_th}   {year_th}'
