...
def month_key(month_th, year_th):
    months = ['มกราคม','กุมภาพันธ์','มีนาคม','เมษายน','พฤษภาคม','มิถุนายน',
              'กรกฎาคม','สิงหาคม','กันยายน','ตุลาคม','พฤศจิกายน','ธันวาคม']
    m = months.index(month_th) + 1 if month_th in months else 0
    return f'{year_th}_{m:02d}'

def month_dir(mk):
    d = os.path.join(SAVE_DIR, mk)
    os.makedirs(d, exist_ok=True)
    return d

def meta_file(mk):
    return os.path.join(month_dir(mk), 'meta.json')
...
def delete_from_cloudinary(public_id):
    try:
        ...
        cloudinary.uploader.destroy(f'lvsb_photos/{public_id}')
    except Exception as e:
        print(f'Cloudinary delete error: {e}')

def download_from_cloudinary(url):
    """Download image from Cloudinary URL, return bytes or None"""
    try:
        ...
        # ดักจับเงื่อนไขไม่มีไฟล์อย่างสุภาพ ไม่ปล่อยให้เกิดการ crash หรือแสดงข้อความ error รบกวนหน้าเว็บ
        if e.code == 404:
            return None
        print(f'Cloudinary download HTTP error: {e}')
        return None
    except Exception as e:
        print(f'Cloudinary download error: {e}')
        return None

# ── Cloudinary Metadata Synchronizer ──
def upload_meta_to_cloudinary(meta, mk):
    """ฟังก์ชันอัปโหลด meta.json ขึ้นไปเก็บบน Cloudinary เป็นไฟล์ข้อมูลสำรองถาวร"""
    ...
    try:
        ...
        # อัปโหลดไฟล์ json ไปที่ Cloudinary ในรูปแบบดิบ (raw resource)
        cloudinary.uploader.upload(
            meta_bytes,
            public_id=f'meta_{mk}.json',
            folder='lvsb_metadata',
            overwrite=True,
            resource_type='raw'
        )
        print(f'[Cloudinary Metadata] Saved meta_{mk}.json to Cloudinary successfully')
    except Exception as e:
        print(f'[Cloudinary Metadata] Failed to save meta_{mk}.json: {e}')

def download_meta_from_cloudinary(mk):
    """ฟังก์ชันวิ่งไปดาวน์โหลดก้อน meta.json มาจาก Cloudinary เพื่อกู้คืนสถานะของ Render"""
    if not cloudinary_enabled():
        return None
    try:
        url = f'https://res.cloudinary.com/{CLOUDINARY_CLOUD_NAME}/raw/upload/lvsb_metadata/meta_{mk}.json'
        data = download_from_cloudinary(url)
        if data:
            meta = json.loads(data.decode('utf-8'))
            print(f'[Cloudinary Metadata] Restored meta_{mk}.json from Cloudinary')
            return meta
    except Exception as e:
        # แสดง log แบบนุ่มนวลกรณีที่ยังไม่มีไฟล์ประวัติของเดือนนั้นๆ
        print(f'[Cloudinary Metadata] No backup file meta_{mk}.json found on Cloudinary (Normal for first-time use)')
    return None
...
LVSBS = [
    ...
    {'id': '14',  'name': 'SS6-1-LVSB2',                   'sheet': '14_SS6-1-LVSB2 '}, # สะกดตรงกับในไฟล์จริง
    ...
    {'id': '30',  'name': 'SS7-2-LVSB2',                   'sheet': '30_SS7-2-LVSB2 '}, # สะกดตรงกับในไฟล์จริง
    ...
    {'id': '34',  'name': 'SS7-3-LVSB2',                   'sheet': '34_SS7-3-LVSB2 '}, # สะกดตรงกับในไฟล์จริง
    ...
    {'id': '65',  'name': 'Unit Sub 1',                    'sheet': '65_Unit Sub 1 '}, # สะกดตรงกับในไฟล์จริง
    {'id': '66',  'name': 'Unit Sub 2',                    'sheet': '66_Unit Sub 2 '}, # สะกดตรงกับในไฟล์จริง
    {'id': '67',  'name': 'Field Sub 1',                   'sheet': '67_Field Sub 1 '}, # สะกดตรงกับในไฟล์จริง
    {'id': '68',  'name': 'Field Sub 2',                   'sheet': '68_Field Sub 2 '}, # สะกดตรงกับในไฟล์จริง
    {'id': '69',  'name': 'Field Sub 3',                   'sheet': '69_Field Sub 3 '}, # สะกดตรงกับในไฟล์จริง
    {'id': '70',  'name': 'Field Sub 4',                   'sheet': '70_Field Sub 4 '}, # สะกดตรงกับในไฟล์จริง
    ...
    {'id': '74',  'name': 'Field Sub 9',                   'sheet': '74_Field Sub 9 '}, # สะกดตรงกับในไฟล์จริง
    {'id': '75',  'name': 'Field Sub 11',                  'sheet': '75_Field Sub 11'},
    {'id': '76',  'name': 'Field Sub 14',                  'sheet': '76_Field Sub 14'},
    {'id': '77',  'name': 'Field Sub 15',                  'sheet': '77_Field Sub 15 '}, # สะกดตรงกับในไฟล์จริง
    ...
    {'id': '102', 'name': 'อาคารพักเวร / MDB',            'sheet': '102_ห้องเวร_MDB'}, # ตรงตามจริงใน Excel
    ...
    {'id': '109', 'name': 'P1 / MDB-P1-B',                 'sheet': '109_P1_ MDB-P1-B'}, # มี Space หลัง _
...
# ── Meta helpers (With Cloudinary Backup/Restore) ────────────────────
def load_meta(mk=None):
    now = datetime.now()
    months_th = ['มกราคม','กุมภาพันธ์','มีนาคม','เมษายน','พฤษภาคม','มิถุนายน',
                 'กรกฎาคม','สิงหาคม','กันยายน','ตุลาคม','พฤศจิกายน','ธันวาคม']
    
    if mk is None:
        month_th = months_th[now.month-1]
        year_th  = str(now.year+543)
        mk = month_key(month_th, year_th)
    else:
        # ดึงเดือน/ปี กลับมาจาก month_key (เช่น 2569_06)
        parts = mk.split('_')
        if len(parts) == 2:
            year_th = parts[0]
            try:
                m_idx = int(parts[1]) - 1
                month_th = months_th[m_idx] if 0 <= m_idx < 12 else ''
            except:
                month_th = ''
        else:
            month_th, year_th = '', ''
    ...
    # 1. เช็กบน Local Disk ของ Render ก่อน
    if os.path.exists(mf):
        try:
            with open(mf, 'r', encoding='utf-8') as f:
                return json.load(f)
        except Exception as e:
            print(f'Error reading local meta: {e}')

    # 2. หากหายไป (จากการ Restart ของ Render) ให้วิ่งไปดึงมาจาก Cloudinary เพื่อกู้คืนข้อมูล
    cloud_meta = download_meta_from_cloudinary(mk)
    if cloud_meta:
        # เขียนสำรองกลับลงดิสก์ Local ไว้ใช้ชั่วคราว
        os.makedirs(os.path.dirname(mf), exist_ok=True)
        with open(mf, 'w', encoding='utf-8') as f:
            json.dump(cloud_meta, f, ensure_ascii=False, indent=2)
        return cloud_meta

    # 3. หากไม่มีทั้งสองแห่ง ค่อยสรุปเป็นข้อมูลเริ่มต้นเปล่า ๆ
    return {'month_th': month_th, 'year_th': year_th, 'lvsbs': {}}

def save_meta(meta, mk=None):
    if mk is None:
        mk = month_key(meta.get('month_th',''), meta.get('year_th',''))
    
    # เซฟลงเครื่อง Local ก่อน
    mf = meta_file(mk)
    os.makedirs(os.path.dirname(mf), exist_ok=True)
    with open(mf, 'w', encoding='utf-8') as f:
        json.dump(meta, f, ensure_ascii=False, indent=2)
        
    # อัปโหลดก้อน meta สำรองขึ้นระบบ Cloudinary ทันทีเพื่อความคงทนถาวร 100%
    upload_meta_to_cloudinary(meta, mk)


# ── Server Helpers ───────────────────────────────────────────────────
def img_dir(lid, mk='default'):
    d = os.path.join(SAVE_DIR, mk, f'lvsb_{lid}')
    os.makedirs(d, exist_ok=True)
    return d

def list_months():
    """ดึงข้อมูลรายการเดือนที่บันทึกไว้ ค้นหาทั้งใน Local และโฟลเดอร์ของแอป"""
    if not os.path.exists(SAVE_DIR):
        return []
    dirs = [d for d in os.listdir(SAVE_DIR)
            if os.path.isdir(os.path.join(SAVE_DIR, d)) and '_' in d]
...
def save_lvsb():
    ...
    mth = request.form.get('month_th', '')
    yr  = request.form.get('year_th', '')
    mk  = month_key(mth, yr)
    meta = load_meta(mk)
    ...
    for f in request.files.getlist('photos'):
        ...
        # บันทึกภาพสำรองลงเซิร์ฟเวอร์แบบ Local ตลอดเวลาเพื่อป้องกันปัญหา Export แล้วตารางว่าง
...
def delete_image():
    ...
    mth   = data.get('month_th', '')
    yr    = data.get('year_th', '')
    mk    = month_key(mth, yr)
    meta  = load_meta(mk)
    entry = meta.get('lvsbs', {}).get(lid, {})
    imgs  = entry.get('images', [])
    img_info = next((i for i in imgs if (i['fname'] if isinstance(i, dict) else i) == fname), None)
    if img_info:
        if isinstance(img_info, dict):
            if img_info.get('public_id'):
                delete_from_cloudinary(img_info['public_id'])
            elif img_info.get('drive_id'):
                delete_from_drive(img_info['drive_id'])
        
        # ลบไฟล์ที่ Local storage
        fpath = os.path.join(img_dir(lid, mk), fname)
...
@app.route('/get_state')
def get_state():
    mth = request.args.get('month_th', '')
    yr  = request.args.get('year_th', '')
    mk  = month_key(mth, yr) if (mth and yr) else None
    meta = load_meta(mk)
    meta['saved_months'] = list_months()
    for lid, entry in meta.get('lvsbs', {}).items():
        valid_filenames = []
        cur_mk = mk or month_key(meta.get('month_th',''), meta.get('year_th',''))
        
        # ตรวจสอบและส่งเฉพาะ String ชื่อไฟล์กลับไปให้ฝั่งหน้าบ้าน (ป้องกันปัญหา [object Object])
        for img in entry.get('images', []):
            fname = img.get('fname', '') if isinstance(img, dict) else img
            # ถึงแม้ไม่มีไฟล์ที่ Local (Render ลบไฟล์) แต่ถ้ามีคีย์ Cloudinary ถือว่ารูปภาพยังปลอดภัยและพร้อมใช้งาน
        ...
        entry['images'] = valid_filenames
    return jsonify(meta)

@app.route('/image/<mk>/<lid>/<fname>')
def serve_image(mk, lid, fname):
    meta     = load_meta(mk)
    entry    = meta.get('lvsbs', {}).get(lid, {})
    
    img_info = None
    for i in entry.get('images', []):
        curr_name = i.get('fname') if isinstance(i, dict) else i
        if curr_name == fname:
            img_info = i
            break

    # 1. เช็ก Cloudinary (Redirect ไปดึงภาพ)
    if img_info and isinstance(img_info, dict) and img_info.get('cloudinary_url'):
        from flask import redirect
        return redirect(img_info['cloudinary_url'])
        
    # 2. เช็ก Google Drive
    if img_info and isinstance(img_info, dict) and img_info.get('drive_id'):
        data = download_from_drive(img_info['drive_id'])
        if data:
            return send_file(io.BytesIO(data), mimetype='image/jpeg')
            
    # 3. เช็ก Local Storage เครื่องเซิร์ฟเวอร์
    fpath = os.path.join(img_dir(lid, mk), fname)
    if os.path.exists(fpath):
...
def reset():
    ...
    if mth and yr:
        ...
        if os.path.exists(d):
            shutil.rmtree(d)
        # เคลียร์ค่าบน Cloudinary ของเดือนนี้ด้วย
        if cloudinary_enabled():
            try:
                import cloudinary.uploader
                cloudinary.uploader.destroy(f'lvsb_metadata/meta_{mk}.json', resource_type='raw')
            except Exception as e:
                print(f'Metadata destroy error on reset: {e}')
    else:
...
def export():
    mth      = request.args.get('month_th', '')
    yr       = request.args.get('year_th', '')
    lid_only = request.args.get('lid', '')   # empty = export all
    mk       = month_key(mth, yr) if (mth and yr) else None
    meta     = load_meta(mk)
    ...
    # เก็บ Path ของไฟล์ Temp ที่ถูกใช้ขณะประมวลผล เพื่อลบทิ้งทีหลังได้อย่างถูกต้อง
    temp_files = []

    # ฟังก์ชันตัวช่วยดึงชื่อชีตจาก workbook แบบยืดหยุ่นและรองรับความผิดพลาด (Whitespace-insensitive matching)
    def find_actual_sheet(wb_obj, expected_name):
        # 1. ลองค้นหาตรงๆ ก่อนเพื่อความรวดเร็ว
        if expected_name in wb_obj.sheetnames:
            return wb_obj[expected_name]
        # 2. ปรับการสะกดแบบยืดหยุ่น (ตัดช่องว่าง, ข้ามความแตกต่างของสัญลักษณ์ - และ _)
        normalized_expected = expected_name.replace(" ", "").replace("-", "_").lower()
        for name in wb_obj.sheetnames:
            normalized_actual = name.replace(" ", "").replace("-", "_").lower()
            if normalized_expected == normalized_actual:
                return wb_obj[name]
        return None

    # หากเลือก Export เฉพาะตู้เดียว ให้ทำการลบชีตของตู้อื่นๆ ออกทั้งหมด เหลือไว้เพียงชีต Main และชีตของตู้ที่เลือก
    if lid_only:
        target_lvsb = next((l for l in LVSBS if l['id'] == lid_only), None)
        keep_sheet_title = None
        if target_lvsb:
            matched_ws = find_actual_sheet(wb, target_lvsb['sheet'])
            if matched_ws:
                keep_sheet_title = matched_ws.title
        
        # ลบชีตทั้งหมดที่ไม่ใช่ชีต 'Main' และไม่ใช่ชีตของตู้ที่ต้องการจะส่งออก
        for sheet_name in list(wb.sheetnames):
            if sheet_name != 'Main' and sheet_name != keep_sheet_title:
    ...
    for lvsb in export_list:
        ...
        # เรียกใช้ฟังก์ชันค้นหาแบบยืดหยุ่น ป้องกันปัญหา Sheet mismatch
        ws = find_actual_sheet(wb, sheet_name)
        if ws is None:
            print(f"[Warning] Sheet '{sheet_name}' not found in template.xlsx")
            continue
        ...
        # ดึงรายชื่ออิมเมจในก้อนข้อมูลจริงเพื่อความแม่นยำในการระบุ URL
        actual_images_info = entry.get('images', [])

        for i in range(min(len(actual_images_info), 12)):
            img = actual_images_info[i]
            fname            = img['fname'] if isinstance(img, dict) else img
            cloudinary_url   = img.get('cloudinary_url') if isinstance(img, dict) else None
            drive_id         = img.get('drive_id')       if isinstance(img, dict) else None

            img_data = None
            if cloudinary_url:
                img_data = download_from_cloudinary(cloudinary_url)
            elif drive_id:
                img_data = download_from_drive(drive_id)

            if img_data:
                # สร้างไฟล์ Temp ขึ้นมาอย่างปลอดภัย ห้ามทำลายทิ้งจนกว่าจะบันทึก workbook เสร็จสมบูรณ์
                tmp = tempfile.NamedTemporaryFile(suffix='.jpg', delete=False)
                tmp.write(img_data)
                tmp.close()  # ปิด handle เพื่อให้ openpyxl เข้าถึงไฟล์ได้ทันที
                tmp_path = tmp.name
                ...
                # กรณีหาไฟล์ที่ดึงสดไม่เจอ ให้ลองไปดึงที่โฟลเดอร์ Local บนเซิร์ฟเวอร์
    ...
    out = io.BytesIO()
    try:
        # บันทึก Workbook ข้อมูลรูปภาพทั้งหมดจะเริ่มเขียนและฝังลง Zip Archive ตอนนี้
        wb.save(out)
    finally:
        # กำจัดไฟล์ Temp หลังจากสั่งเซฟและแพ็คไฟล์ Excel เรียบร้อยแล้ว
    ...
    if lid_only:
        ...
        fname_dl  = f'Energy_{lvsb_name}_{month_th}_{year_th}.xlsx'
    else:
        fname_dl  = f'Energy_LVSB_{month_th}_{year_th}.xlsx'
    return send_file(out, as_attachment=True,
                     download_name=fname_dl,
