...
def delete_image():
    ...
    if img_info:
        if isinstance(img_info, dict) and img_info.get('drive_id'):
...
def serve_image(lid, fname):
    meta  = load_meta()
    entry = meta.get('lvsbs', {}).get(lid, {})
    img_info = next((i for i in entry.get('images', []) if (i['fname'] if isinstance(i, dict) else i) == fname), None)
    print(f'[serve] lid={lid} fname={fname} img_info={img_info}')
    if img_info and isinstance(img_info, dict) and img_info.get('drive_id'):
        data = download_from_drive(img_info['drive_id'])
        if data:
            return send_file(io.BytesIO(data), mimetype='image/jpeg')
        print(f'[serve] drive download failed for {img_info["drive_id"]}')
    fpath = os.path.join(img_dir(lid), fname)
    if os.path.exists(fpath):
        return send_file(fpath, mimetype='image/jpeg')
    print(f'[serve] not found locally either: {fpath}')
