...
# รายชื่อตู้ที่ปรับปรุงการสะกดชื่อ Sheet ให้ตรงกับชีตจริงในไฟล์ template.xlsx 100%
LVSBS = [
...
def load_meta(mk=None):
    ...
    if os.path.exists(mf):
        with open(mf, 'r', encoding='utf-8') as f:
            return json.load(f)
    return {'month_th': '', 'year_th': '', 'lvsbs': {}}

def save_meta(meta, mk=None):
    if mk is None:
        mk = month_key(meta.get('month_th',''), meta.get('year_th',''))
    with open(meta_file(mk), 'w', encoding='utf-8') as f:
...
def list_months():
    """Return list of saved month keys sorted descending"""
...
def export():
    ...
    for lvsb in export_list:
        ...
        for i, img in enumerate(entry.get('images', [])[:12]):
